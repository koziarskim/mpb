...
set SCHEMA_FILE_SHARED=%EXPORT_DIR%\schema_shared_%DATE_FORMAT%.sql
set SCHEMA_FILE_2020=%EXPORT_DIR%\schema_2020_%DATE_FORMAT%.sql
set DATA_FILE_SHARED=%EXPORT_DIR%\data_shared_%DATE_FORMAT%.sql
...
if "%IMPORT%" == "true" (
	...
	findstr /v /b /c:"SET " /c:"CREATE DATABASE" /c:"ALTER DATABASE" /c:"\connect" /c:"REVOKE ALL" /c:"GRANT ALL" /c:"SELECT pg_catalog.set_config('search_path'" %SCHEMA_FILE_SHARED% > %EXPORT_DIR%\clean_schema_shared.sql
