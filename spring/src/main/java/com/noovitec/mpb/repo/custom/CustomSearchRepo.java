...
public interface CustomSearchRepo {
	...
	public class SearchRepoImpl implements CustomSearchRepo {
		...
		public List<KeyValueDto> findCustomers(SearchDto searchDto) {
			// TODO: This could go to client site.
			if ((searchDto.getSeasons().isEmpty() && !searchDto.isSeasonAll()) || (searchDto.getItems().isEmpty() && !searchDto.isItemAll())) {
				return new ArrayList<KeyValueDto>();
			}
		...
		public List<KeyValueDto> findSuppliers(SearchDto searchDto) {
			// TODO: This could go to client site.
			if ((searchDto.getSeasons().isEmpty() && !searchDto.isSeasonAll()) || (searchDto.getItems().isEmpty() && !searchDto.isItemAll())
					|| (searchDto.getCustomers().isEmpty() && !searchDto.isCustomerAll()) || (searchDto.getSales().isEmpty() && !searchDto.isSaleAll())) {
				return new ArrayList<KeyValueDto>();
			}
		...
		public List<PoComponentDto> findPoComponents(SearchDto searchDto) {
			...
			for (Component c : components) {
				...
				BigDecimal totalPrice = (dto.getUnitPrice() == null || dto.getUnits() == null) ? BigDecimal.ZERO
						: dto.getUnitPrice().multiply(BigDecimal.valueOf(dto.getUnits(), 2));
