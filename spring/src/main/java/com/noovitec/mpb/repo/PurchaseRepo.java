...
import com.noovitec.mpb.repo.custom.CustomPurchaseRepo;

public interface PurchaseRepo extends JpaRepository<Purchase, Long>, CustomPurchaseRepo {

	@Query("select distinct p from Purchase p where p.id in :purchaseIds")
	List<Purchase> findByIds(List<Long> purchaseIds);
