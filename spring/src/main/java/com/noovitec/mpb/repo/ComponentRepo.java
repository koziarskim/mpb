...
public interface ComponentRepo extends JpaRepository<Component, Long> {
	...
	@Query("select distinct new com.noovitec.mpb.dto.ComponentDto(" 
			+ "c.id, c.number, c.name, sum((ic.units * si.units)-c.unitsOnStack-c.unitsOrdered-c.unitsInTransit), c.unitsOnStack, pc.units, pc.unitsReceived, c.totalLandedCost, (pc.id is not null)) "
			+ "from Component c " 
			...
			+ "left join c.purchaseComponents pc with pc.purchase.id = ps.purchase.id and pc.component.id = c.id " 
