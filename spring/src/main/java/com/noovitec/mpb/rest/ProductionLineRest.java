...
class ProductionLineRest {
	...
	@GetMapping("/productionLine/dateStarted/{dateStarted}")
	List<ProductionLine> get(@PathVariable(name = "dateStarted") @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate dateStarted) {
		List<ProductionLine> result = productionLineRepo.findByDateStarted(dateStarted);
		return result;
	}
