...
CREATE SEQUENCE public.production_line_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE public.production_line_id_seq OWNER TO postgres;

ALTER SEQUENCE public.production_line_id_seq OWNED BY public.production_line.id;

ALTER TABLE ONLY public.production_line ALTER COLUMN id SET DEFAULT nextval('public.production_line_id_seq'::regclass);

ALTER TABLE ONLY public.production_line
    ADD CONSTRAINT production_line_pkey PRIMARY KEY (id);

ALTER TABLE ONLY public.production_line
    ADD CONSTRAINT production_line_pk_item_id FOREIGN KEY (item_id) REFERENCES public.item(id);

ALTER TABLE ONLY public.production_line
    ADD CONSTRAINT production_line_pk_line_id FOREIGN KEY (line_id) REFERENCES public.line(id);
...
CREATE TABLE public.production_output (
    ...
    updated timestamp without time zone,
    date_produced date,
	time_produced time without time zone,
    units bigint,
...
ALTER TABLE public.production_output_id_seq OWNER TO postgres;
...
ALTER TABLE ONLY public.production_output
    ADD CONSTRAINT production_output_pk_production_line_id FOREIGN KEY (production_line_id) REFERENCES public.production_line(id);
